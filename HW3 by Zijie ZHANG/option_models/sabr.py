...
class ModelHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        ...
        forward = spot * np.exp(texp*(self.intr - self.divr))
        if is_vol==False:
            vol3 = self.bsm_model.impvol(price_or_vol3, strike3, spot, texp, cp_sign=cp_sign)
        iv_func = lambda PA: \
            bsm_vol(strike3, forward, texp, PA[0],PA[1],PA[2],beta=1) - vol3
        para=sopt.root(iv_func,[0.1*forward,0.1,0])
        '''  
...
class ModelNormalHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        ...
        if is_vol==False:
            vol3=[]
            for i in range(0,np.size(price_or_vol3)):
                vol3.append(self.normal_model.impvol(price_or_vol3[i], strike3[i], spot, texp, cp_sign=cp_sign))
            vol3=np.array(vol3)
        else:
            vol3=price_or_vol3
        iv_func = lambda PA : \
            norm_vol(strike3, forward, texp, PA[0],PA[1],PA[2]) - vol3
        para=sopt.root(iv_func,[0.1*forward,0.1,0])
        '''  
...
class ModelBsmMC:
    ...
    def bsm_vol(self, strike, spot, texp=None, sigma=None):
        ...
        return bsm_model.impvol(price,strike,spot,texp,cp_sign=1)
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        Z=np.random.normal(0,1,[1000,np.size(strike)])
        Stexp=spot*np.exp(-1/2*(sigma**2)*texp+sigma*np.sqrt(texp)*Z)
        mcprice=np.mean(np.maximum(Stexp-strike,0),axis=0)
        return mcprice
...
class ModelNormalMC:
    ...
    def norm_vol(self, strike, spot, texp=None, sigma=None):
        ...
        price=self.price(strike,spot,texp,sigma,cp_sign=1)
        return normal_model.impvol(price,strike,spot,texp,cp_sign=1)
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        np.random.seed(12345)
        Z=np.random.normal(0,1,[1000,np.size(strike)])
        Stexp=spot+sigma*np.sqrt(texp)*Z
        mcprice=np.mean(np.maximum(Stexp-strike,0),axis=0)
        return mcprice
...
class ModelBsmCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        np.random.seed(12345)
        N=100
        Z=np.random.normal(0,1,[np.size(strike),1000,N])
        X=np.random.normal(0,1,[np.size(strike),1000])
        sigT_temp=np.exp(-1/2*alpha**2*texp/N+alpha*np.sqrt(texp/N)*Z)
        sigT_temp[:,:,0]=sigT_temp[:,:,0]*sigma
        sigT_matrix=np.cumprod(sigT_temp,axis=2)
        sigT=sigT_matrix[:,:,-1]
        I_T=np.sum(texp/N*sigT_matrix**2,axis=2)
        S_T=spot*np.exp(rho/alpha*(sigT-sigma)-1/2*I_T+np.sqrt(1-rho**2)*X*np.sqrt(I_T))
        S_T=S_T.reshape((1000,np.size(strike)))
        mcprice=np.mean(np.maximum(S_T-strike,0),axis=0)
        return mcprice
...
class ModelNormalCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        sigT=sigT_matrix[:,:,-1]
        I_T=np.sum(texp/N*sigT_matrix**2,axis=2)
        S_T=spot+rho/alpha*(sigT-sigma)+np.sqrt(1-rho**2)*X*np.sqrt(I_T)
        S_T=S_T.reshape((1000,np.size(strike)))
        mcprice=np.mean(np.maximum(S_T-strike,0),axis=0)
